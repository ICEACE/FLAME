...
#Export Firms #
eFneed <- get_aggregate_values(EFirmBalance$IT_NO, EFirmBalance$ISLIQUIDSHORT)
eFloan <- get_aggregate_values(EFirmBalance$IT_NO, EFirmBalance$HASLOAN)
eFinvest <- get_aggregate_values(EFirmBalance$IT_NO, EFirmBalance$HASINVESTMENT)
plot_time_series_multiline_point_file(nQuarters, eFneed, eFloan, "Quarters", "Frequencies", "Exporter Firm Liquidity Needs vs Bank Loans and Fund Investments", "aCredittingEFirm.png", c("Liquidity Shortage", "Loans", "Investment"), v3 = eFinvest, legposn = "topright", ptype = "o")
