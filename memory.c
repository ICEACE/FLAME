...
void unittest_idle_FirmConsumptionStart_FirmExportStart()
{
...
void unittest_idle_FirmConsumptionMarket_FirmExportStart()
{
...
void unittest_firm_consumption_update_market_price_FirmConsumptionPrice_FirmExportStart()
{
	int rc;
...
void unittest_idle_FirmProductionRoleCheck_FirmProductionExport()
{
...
void unittest_firm_production_produce_export_goods_FirmProductionExport_FirmLabourStart()
{
	
	
	//return firm_production_produce_export_goods();
}

void unittest_firm_production_export_plan_FirmProductionExport_FirmExportLabourDemand()
{
	
	
	//return firm_production_export_plan();
}

void unittest_firm_production_export_labour_demand_FirmExportLabourDemand_FirmLabourStart()
{
	
	
	//return firm_production_export_labour_demand();
}
...
void unittest_idle_FirmExportStart_FirmHousingStart()
{
...
void unittest_idle_FirmExportStart_FirmExportMarket()
{
...
void unittest_idle_FirmExportMarket_FirmHousingStart()
{
...
void unittest_firm_export_sell_FirmExportMarket_FirmExportReceiveSales()
{
	
	
	//return firm_export_sell();
}

void unittest_firm_export_receive_sales_FirmExportReceiveSales_FirmHousingStart()
{
	int rc;
	
	rc = MB_Iterator_CreateFiltered(b_centralbank_firm_revenues, &i_centralbank_firm_revenues, &FLAME_filter_firm_firm_export_receive_sales_FirmExportReceiveSales_FirmHousingStart_centralbank_firm_revenues, current_xmachine_firm);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'centralbank_firm_revenues'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'centralbank_firm_revenues' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return firm_export_receive_sales();
}
...
void unittest_idle_CentralBankConsumptionPrices_CentralBankExport()
{
...
void unittest_centralbank_update_price_indices_CentralBankConsumptionPrices_CentralBankExport()
{
...
void unittest_centralbank_export_transactions_CentralBankExport_CentralBankHousing()
{
	int rc;
	
	
	rc = MB_Iterator_Create(b_foreignsector_centralbank_revenues, &i_foreignsector_centralbank_revenues);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'foreignsector_centralbank_revenues'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'foreignsector_centralbank_revenues' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return centralbank_export_transactions();
}

void unittest_idle_CentralBankExport_CentralBankHousing()
{
...
void unittest_foreignsector_init_start_ForeignsectorLoop()
{
	
	
	//return foreignsector_init();
}

void unittest_idle_start_ForeignsectorLoop()
{
...
void unittest_foreignsector_iterate_ForeignsectorLoop_ForeignSectorExportsStart()
{
	
	
	//return foreignsector_iterate();
}

void unittest_foreignsector_export_buy_ForeignSectorExportsStart_ForeignSectorExportsPricing()
{
	int rc;
	
	
	rc = MB_Iterator_Create(b_firm_foreignsector_amount, &i_firm_foreignsector_amount);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_foreignsector_amount'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_foreignsector_amount' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return foreignsector_export_buy();
}

void unittest_idle_ForeignSectorExportsStart_end()
{
...
void unittest_foreignsector_export_pricing_ForeignSectorExportsPricing_ForeignSectorExportsPayment()
{
	
	
	//return foreignsector_export_pricing();
}

void unittest_foreignsector_export_pay_ForeignSectorExportsPayment_end()
{
	
	
	//return foreignsector_export_pay();
}
...
void free_messages()
{
	int rc;
	    ...
	    rc = MB_Clear(b_firm_foreignsector_amount);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_foreignsector_amount' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_foreignsector_amount' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_centralbank_firm_revenues);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'centralbank_firm_revenues' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'centralbank_firm_revenues' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_foreignsector_centralbank_revenues);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'foreignsector_centralbank_revenues' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'foreignsector_centralbank_revenues' board is invalid\n");
	               break;
...
int rc;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_foreignsector_amount_composite_params = NULL;

	    rc = MB_Create(&b_firm_foreignsector_amount, sizeof(m_firm_foreignsector_amount));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_foreignsector_amount' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_centralbank_firm_revenues_composite_params = NULL;

	    rc = MB_Create(&b_centralbank_firm_revenues, sizeof(m_centralbank_firm_revenues));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'centralbank_firm_revenues' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_foreignsector_centralbank_revenues_composite_params = NULL;

	    rc = MB_Create(&b_foreignsector_centralbank_revenues, sizeof(m_foreignsector_centralbank_revenues));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'foreignsector_centralbank_revenues' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	firm_FirmExportReceiveSales_state = init_firm_state();

	firm_FirmExportMarket_state = init_firm_state();
	...
	firm_FirmExportLabourDemand_state = init_firm_state();
	...
	firm_FirmProductionExport_state = init_firm_state();
	...
	firm_FirmExportStart_state = init_firm_state();
	...
	centralbank_CentralBankExport_state = init_centralbank_state();
	...
	foreignsector_ForeignSectorExportsPayment_state = init_foreignsector_state();

	foreignsector_end_state = init_foreignsector_state();

	foreignsector_ForeignSectorExportsPricing_state = init_foreignsector_state();

	foreignsector_ForeignSectorExportsStart_state = init_foreignsector_state();

	foreignsector_ForeignsectorLoop_state = init_foreignsector_state();

	foreignsector_start_state = init_foreignsector_state();
...
xmachine_memory_firm * init_firm_agent()
{
	...
	current->physical_capital_construction = 0;
	current->physical_capital = 0;
...
void unittest_init_firm_agent()
{
	...
	CHECK_POINTER(current);
		...
		current_xmachine_firm->firmtype = 0;
		...
		current_xmachine_firm->physical_capital_construction = 0;
		current_xmachine_firm->physical_capital = 0;
...
{
	current_xmachine_firm_holder = firm_FirmExportReceiveSales_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmExportReceiveSales_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmExportReceiveSales_state->count = 0;
	current_xmachine_firm_holder = firm_FirmExportMarket_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmExportMarket_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmExportMarket_state->count = 0;
	...
	current_xmachine_firm_holder = firm_FirmExportLabourDemand_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmExportLabourDemand_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmExportLabourDemand_state->count = 0;
	...
	current_xmachine_firm_holder = firm_FirmProductionExport_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmProductionExport_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmProductionExport_state->count = 0;
	...
	current_xmachine_firm_holder = firm_FirmExportStart_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmExportStart_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmExportStart_state->count = 0;
...
{
	free(firm_FirmExportReceiveSales_state);
	free(firm_FirmExportMarket_state);
	...
	free(firm_FirmExportLabourDemand_state);
	...
	free(firm_FirmProductionExport_state);
	...
	free(firm_FirmExportStart_state);
 ...
 * \param firmtype Variable for the X-machine memory.
 ...
 * \param physical_capital_construction Variable for the X-machine memory.
 ...
 */
void add_firm_agent(int id, int bank_id, int firmtype, int day_of_month_to_act, int isinsolvent, int it_no, int day_of_week_to_act, double average_goods_price, int_array * employees, double wage_offer, double average_wage, int no_employees, int vacancies, int employees_needed, int day_of_month_wages_paid, double labour_productivity, int production_current, int expected_sales, int production_plan, double unit_goods_price, double unit_cost, int day_of_month_production_completed, double unit_house_price, double labour_productivity_construction, double capital_productivity_construction, int physical_capital_construction, int physical_capital, int projects[], double loans_interest_rate, double debt, int inventory, int sales, double revenues, double total_assets, double operating_costs, double labour_costs, double total_interest_payments, double dividends_paid, double dividends_to_be_paid, double retained_earnings, double net_earnings, double ebit, double equity, double liquidity, int capital_goods, double capital_goods_price, int isliquidshort, int hasloan, int hasinvestment, int isilliquid, double planned_investment_costs, double liquidity_need, loan loan_list[], double labour_tax_rate, double delta_housing_price)
{
	...
	current->physical_capital_construction = physical_capital_construction;
...
/** \fn void add_household_agent(int id, int bank_id, int it_no, int day_of_week_to_act, double weekly_consumption_budget, double mall_budget, int my_employer_id, double wage, double government_benefits, int day_of_month_to_act, int day_of_month_wage_recieved, double mortgages_interest_rate, double labour_tax_rate, mortgage_array * mortgages_list, double mortgages, double housing_payment, double equity, double housing_price, int housing_units, int n_shares, double liquidity, double capital_income, double previous_wages[], double previous_benefits[], double labour_income, double total_assets, double housing_value, double expected_housing_payment, int hmarket_role, double equity_ratio, double minimum_equity_ratio, double mortgage_costs[], double delta_housing_value)
...
void free_centralbank_agents()
{
	...
	current_xmachine_centralbank_holder = centralbank_CentralBankExport_state->agents;
	while(current_xmachine_centralbank_holder)
	{
		temp_xmachine_centralbank_holder = current_xmachine_centralbank_holder->next;
		free_centralbank_agent(current_xmachine_centralbank_holder, centralbank_CentralBankExport_state);
		current_xmachine_centralbank_holder = temp_xmachine_centralbank_holder;
	}
	centralbank_CentralBankExport_state->count = 0;
...
void free_centralbank_states()
{
	...
	free(centralbank_CentralBankExport_state);
...
xmachine_memory_foreignsector_state * init_foreignsector_state()
{
	xmachine_memory_foreignsector_state * current = (xmachine_memory_foreignsector_state *)malloc(sizeof(xmachine_memory_foreignsector_state));
	CHECK_POINTER(current);
...
xmachine_memory_foreignsector * init_foreignsector_agent()
{
	xmachine_memory_foreignsector * current = (xmachine_memory_foreignsector *)malloc(sizeof(xmachine_memory_foreignsector));
	CHECK_POINTER(current);

	current->id = 0;
	current->it_no = 0;
	current->day_of_month_to_act = 0;
	current->xgoods_unit_price = 0.0;

	return current;
}

void free_foreignsector_agent(xmachine_memory_foreignsector_holder * tmp, xmachine_memory_foreignsector_state * state)
{
	if(tmp->prev == NULL) state->agents = tmp->next;
	else tmp->prev->next = tmp->next;
	if(tmp->next != NULL) tmp->next->prev = tmp->prev;
...
void unittest_init_foreignsector_agent()
{
	current_xmachine_foreignsector = (xmachine_memory_foreignsector *)malloc(sizeof(xmachine_memory_foreignsector));
	CHECK_POINTER(current);

		current_xmachine_foreignsector->id = 0;
		current_xmachine_foreignsector->it_no = 0;
		current_xmachine_foreignsector->day_of_month_to_act = 0;
		current_xmachine_foreignsector->xgoods_unit_price = 0.0;
	
}

void unittest_free_foreignsector_agent()
{
	
	free(current_xmachine_foreignsector);
}

void free_foreignsector_agents()
{
	current_xmachine_foreignsector_holder = foreignsector_ForeignSectorExportsPayment_state->agents;
	while(current_xmachine_foreignsector_holder)
	{
		temp_xmachine_foreignsector_holder = current_xmachine_foreignsector_holder->next;
		free_foreignsector_agent(current_xmachine_foreignsector_holder, foreignsector_ForeignSectorExportsPayment_state);
		current_xmachine_foreignsector_holder = temp_xmachine_foreignsector_holder;
	}
	foreignsector_ForeignSectorExportsPayment_state->count = 0;
	current_xmachine_foreignsector_holder = foreignsector_end_state->agents;
	while(current_xmachine_foreignsector_holder)
	{
		temp_xmachine_foreignsector_holder = current_xmachine_foreignsector_holder->next;
		free_foreignsector_agent(current_xmachine_foreignsector_holder, foreignsector_end_state);
		current_xmachine_foreignsector_holder = temp_xmachine_foreignsector_holder;
	}
	foreignsector_end_state->count = 0;
	current_xmachine_foreignsector_holder = foreignsector_ForeignSectorExportsPricing_state->agents;
	while(current_xmachine_foreignsector_holder)
	{
		temp_xmachine_foreignsector_holder = current_xmachine_foreignsector_holder->next;
		free_foreignsector_agent(current_xmachine_foreignsector_holder, foreignsector_ForeignSectorExportsPricing_state);
		current_xmachine_foreignsector_holder = temp_xmachine_foreignsector_holder;
	}
	foreignsector_ForeignSectorExportsPricing_state->count = 0;
	current_xmachine_foreignsector_holder = foreignsector_ForeignSectorExportsStart_state->agents;
	while(current_xmachine_foreignsector_holder)
	{
		temp_xmachine_foreignsector_holder = current_xmachine_foreignsector_holder->next;
		free_foreignsector_agent(current_xmachine_foreignsector_holder, foreignsector_ForeignSectorExportsStart_state);
		current_xmachine_foreignsector_holder = temp_xmachine_foreignsector_holder;
	}
	foreignsector_ForeignSectorExportsStart_state->count = 0;
	current_xmachine_foreignsector_holder = foreignsector_ForeignsectorLoop_state->agents;
	while(current_xmachine_foreignsector_holder)
	{
		temp_xmachine_foreignsector_holder = current_xmachine_foreignsector_holder->next;
		free_foreignsector_agent(current_xmachine_foreignsector_holder, foreignsector_ForeignsectorLoop_state);
		current_xmachine_foreignsector_holder = temp_xmachine_foreignsector_holder;
	}
	foreignsector_ForeignsectorLoop_state->count = 0;
	current_xmachine_foreignsector_holder = foreignsector_start_state->agents;
	while(current_xmachine_foreignsector_holder)
	{
		temp_xmachine_foreignsector_holder = current_xmachine_foreignsector_holder->next;
		free_foreignsector_agent(current_xmachine_foreignsector_holder, foreignsector_start_state);
		current_xmachine_foreignsector_holder = temp_xmachine_foreignsector_holder;
	}
	foreignsector_start_state->count = 0;
}

void free_foreignsector_states()
{
	free(foreignsector_ForeignSectorExportsPayment_state);
	free(foreignsector_end_state);
	free(foreignsector_ForeignSectorExportsPricing_state);
	free(foreignsector_ForeignSectorExportsStart_state);
	free(foreignsector_ForeignsectorLoop_state);
	free(foreignsector_start_state);
}

void transition_foreignsector_agent(xmachine_memory_foreignsector_holder * tmp, xmachine_memory_foreignsector_state * from_state, xmachine_memory_foreignsector_state * to_state)
{
	if(tmp->prev == NULL) from_state->agents = tmp->next;
	else tmp->prev->next = tmp->next;
	if(tmp->next != NULL) tmp->next->prev = tmp->prev;

	add_foreignsector_agent_internal(tmp->agent, to_state);
	free(tmp);
}

void add_foreignsector_agent_internal(xmachine_memory_foreignsector * agent, xmachine_memory_foreignsector_state * state)
{
	xmachine_memory_foreignsector_holder * current = (xmachine_memory_foreignsector_holder *)malloc(sizeof(xmachine_memory_foreignsector_holder));
	CHECK_POINTER(current);
...
/** \fn void add_foreignsector_agent(int id, int it_no, int day_of_month_to_act, double xgoods_unit_price)
 * \brief Add foreignsector X-machine to the current being used X-machine list.
 * \param id Variable for the X-machine memory.
 * \param it_no Variable for the X-machine memory.
 * \param day_of_month_to_act Variable for the X-machine memory.
 * \param xgoods_unit_price Variable for the X-machine memory.
 */
void add_foreignsector_agent(int id, int it_no, int day_of_month_to_act, double xgoods_unit_price)
{
	xmachine_memory_foreignsector * current;

	current = init_foreignsector_agent();
	/* new line added to handle dynamic agent creation*/
	current_xmachine_foreignsector_next_state = foreignsector_start_state;
	add_foreignsector_agent_internal(current, current_xmachine_foreignsector_next_state);

	current->id = id;
	current->it_no = it_no;
	current->day_of_month_to_act = day_of_month_to_act;
	current->xgoods_unit_price = xgoods_unit_price;
}
...
void set_id(int id)
{
	...
	if(current_xmachine->xmachine_foreignsector) (*current_xmachine->xmachine_foreignsector).id = id;
...
int get_id()
{
	...
	if(current_xmachine->xmachine_foreignsector) return (*current_xmachine->xmachine_foreignsector).id;
...
/** \fn void set_firmtype(int firmtype)
 * \brief Set firmtype memory variable for current X-machine.
 * \param firmtype New value for variable.
 */
void set_firmtype(int firmtype)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).firmtype = firmtype;
}

/** \fn int get_firmtype()
 * \brief Get firmtype memory variable from current X-machine.
 * \return Value for variable.
 */
int get_firmtype()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).firmtype;
...
void set_day_of_month_to_act(int day_of_month_to_act)
{
	...
	if(current_xmachine->xmachine_foreignsector) (*current_xmachine->xmachine_foreignsector).day_of_month_to_act = day_of_month_to_act;
...
int get_day_of_month_to_act()
{
	...
	if(current_xmachine->xmachine_foreignsector) return (*current_xmachine->xmachine_foreignsector).day_of_month_to_act;
...
void set_it_no(int it_no)
{
	...
	if(current_xmachine->xmachine_foreignsector) (*current_xmachine->xmachine_foreignsector).it_no = it_no;
...
int get_it_no()
{
	...
	if(current_xmachine->xmachine_foreignsector) return (*current_xmachine->xmachine_foreignsector).it_no;
...
/** \fn void set_physical_capital_construction(int physical_capital_construction)
 * \brief Set physical_capital_construction memory variable for current X-machine.
 * \param physical_capital_construction New value for variable.
 */
void set_physical_capital_construction(int physical_capital_construction)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).physical_capital_construction = physical_capital_construction;
}

/** \fn int get_physical_capital_construction()
 * \brief Get physical_capital_construction memory variable from current X-machine.
 * \return Value for variable.
 */
int get_physical_capital_construction()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).physical_capital_construction;
...
/** \fn void set_xgoods_unit_price(double xgoods_unit_price)
 * \brief Set xgoods_unit_price memory variable for current X-machine.
 * \param xgoods_unit_price New value for variable.
 */
void set_xgoods_unit_price(double xgoods_unit_price)
{
	if(current_xmachine->xmachine_foreignsector) (*current_xmachine->xmachine_foreignsector).xgoods_unit_price = xgoods_unit_price;
}

/** \fn double get_xgoods_unit_price()
 * \brief Get xgoods_unit_price memory variable from current X-machine.
 * \return Value for variable.
 */
double get_xgoods_unit_price()
{
	if(current_xmachine->xmachine_foreignsector) return (*current_xmachine->xmachine_foreignsector).xgoods_unit_price;
...
double agent_get_range()
{
    ...
    /*if (current_xmachine->xmachine_foreignsector) value = current_xmachine->xmachine_foreignsector->;*/
...
int agent_get_id()
{
    ...
    /*if (current_xmachine->xmachine_foreignsector) value = current_xmachine->xmachine_foreignsector->id;*/
...
double agent_get_x()
{
    ...
    /*if (current_xmachine->xmachine_foreignsector) value = current_xmachine->xmachine_foreignsector->0.0;*/
...
void add_node(int node_id, double minx, double maxx, double miny, double maxy, double minz, double maxz)
{
	...
	current->firm_foreignsector_amount_messages = NULL;
	current->centralbank_firm_revenues_messages = NULL;
	current->foreignsector_centralbank_revenues_messages = NULL;
...
void clean_up(int code)
{
	...
	rc = MB_Delete(&b_firm_foreignsector_amount);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_foreignsector_amount' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_centralbank_firm_revenues);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'centralbank_firm_revenues' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_foreignsector_centralbank_revenues);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'foreignsector_centralbank_revenues' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
...
/*	node_information * node_info;
	...
	while(current_xmachine)
	{
		...
		else if(current_xmachine->xmachine_foreignsector != NULL)
		{
			x_xmachine = current_xmachine->xmachine_foreignsector->0.0;
			y_xmachine = current_xmachine->xmachine_foreignsector->0.0;
			z_xmachine = 0.0;
		}
