/**
 * \file  bank_agent_header.h
 * \brief Header for agent type memory access.
 */

/** \def ID
 * \brief Direct access to id of bank agent memory variable. */
#define ID (current_xmachine_bank->id)
/** \def DAY_OF_MONTH_TO_ACT
 * \brief Direct access to day_of_month_to_act of bank agent memory variable. */
#define DAY_OF_MONTH_TO_ACT (current_xmachine_bank->day_of_month_to_act)
/** \def DAY_OF_WEEK_TO_ACT
 * \brief Direct access to day_of_week_to_act of bank agent memory variable. */
#define DAY_OF_WEEK_TO_ACT (current_xmachine_bank->day_of_week_to_act)
/** \def IT_NO
 * \brief Direct access to it_no of bank agent memory variable. */
#define IT_NO (current_xmachine_bank->it_no)
/** \def TOTAL_ASSETS
 * \brief Direct access to total_assets of bank agent memory variable. */
#define TOTAL_ASSETS (current_xmachine_bank->total_assets)
/** \def LOANS
 * \brief Direct access to loans of bank agent memory variable. */
#define LOANS (current_xmachine_bank->loans)
/** \def LOANS_START
 * \brief Direct access to loans_start of bank agent memory variable. */
#define LOANS_START (current_xmachine_bank->loans_start)
/** \def MORTGAGES
 * \brief Direct access to mortgages of bank agent memory variable. */
#define MORTGAGES (current_xmachine_bank->mortgages)
/** \def DEPOSITS
 * \brief Direct access to deposits of bank agent memory variable. */
#define DEPOSITS (current_xmachine_bank->deposits)
/** \def CENTRALBANK_DEBT
 * \brief Direct access to centralbank_debt of bank agent memory variable. */
#define CENTRALBANK_DEBT (current_xmachine_bank->centralbank_debt)
/** \def EQUITY
 * \brief Direct access to equity of bank agent memory variable. */
#define EQUITY (current_xmachine_bank->equity)
/** \def LIQUIDITY
 * \brief Direct access to liquidity of bank agent memory variable. */
#define LIQUIDITY (current_xmachine_bank->liquidity)
/** \def REVENUES
 * \brief Direct access to revenues of bank agent memory variable. */
#define REVENUES (current_xmachine_bank->revenues)
/** \def TOTAL_WRITEOFFS
 * \brief Direct access to total_writeoffs of bank agent memory variable. */
#define TOTAL_WRITEOFFS (current_xmachine_bank->total_writeoffs)
/** \def INTEREST_RATE
 * \brief Direct access to interest_rate of bank agent memory variable. */
#define INTEREST_RATE (current_xmachine_bank->interest_rate)
/** \def INTERESTS_ACCRUED
 * \brief Direct access to interests_accrued of bank agent memory variable. */
#define INTERESTS_ACCRUED (current_xmachine_bank->interests_accrued)
/** \def INTERESTS_PAID
 * \brief Direct access to interests_paid of bank agent memory variable. */
#define INTERESTS_PAID (current_xmachine_bank->interests_paid)
/** \def DIVIDENDS_PAID
 * \brief Direct access to dividends_paid of bank agent memory variable. */
#define DIVIDENDS_PAID (current_xmachine_bank->dividends_paid)
/** \def TOTAL_DIVIDENDS
 * \brief Direct access to total_dividends of bank agent memory variable. */
#define TOTAL_DIVIDENDS (current_xmachine_bank->total_dividends)
/** \def RETAINED_EARNINGS
 * \brief Direct access to retained_earnings of bank agent memory variable. */
#define RETAINED_EARNINGS (current_xmachine_bank->retained_earnings)
/** \def NET_EARNINGS
 * \brief Direct access to net_earnings of bank agent memory variable. */
#define NET_EARNINGS (current_xmachine_bank->net_earnings)
/** \def TOTAL_COSTS
 * \brief Direct access to total_costs of bank agent memory variable. */
#define TOTAL_COSTS (current_xmachine_bank->total_costs)
