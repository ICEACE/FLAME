				...
				int * itno)
{
	...
	int in_physical_capital_construction = 0;
	...
	xmachine_memory_foreignsector * current_foreignsector_agent = NULL;
	...
	while(c != (char)EOF)
	{
		...
		if(c == '>')
		{
			...
			{	
				...
				else if(strcmp(agentname, "foreignsector") == 0)
				{
					if(current_foreignsector_agent == NULL) { printf("Memory error reading foreignsector agent\n"); exit(0); }
					...
					{
						/* Next line should be commented out? */
						add_foreignsector_agent_internal(current_foreignsector_agent, foreignsector_start_state);
			...
			if(strcmp(buffer, "firmtype") == 0) { in_firmtype = 1; }
			if(strcmp(buffer, "/firmtype") == 0) { in_firmtype = 0; }
			...
			if(strcmp(buffer, "physical_capital_construction") == 0) { in_physical_capital_construction = 1; }
			if(strcmp(buffer, "/physical_capital_construction") == 0) { in_physical_capital_construction = 0; }
			...
			if(strcmp(buffer, "xgoods_unit_price") == 0) { in_xgoods_unit_price = 1; }
			if(strcmp(buffer, "/xgoods_unit_price") == 0) { in_xgoods_unit_price = 0; }
		...
		else if(c == '<')
		{
			...
			if(FLAME_in_xagent == 1)
			{
				if(FLAME_in_name == 1)
				{
					...
					{
						current_foreignsector_agent = init_foreignsector_agent();
						in_foreignsector_agent = 1;
					}
					else if(strcmp(agentname, "jpoffice") == 0)
					{
				...
				else if(in_firm_agent == 1)
				{
					...
					if(in_firmtype) { current_firm_agent->firmtype = atoi(buffer); }
					...
					if(in_physical_capital_construction) { current_firm_agent->physical_capital_construction = atoi(buffer); }
					if(in_physical_capital) { current_firm_agent->physical_capital = atoi(buffer); }
					if(in_projects) { j = 0;
				 ...
				 }else if(in_centralbank_agent == 1)
				{
				 ...
				 }else if(in_foreignsector_agent == 1)
				{
					if(in_id) { current_foreignsector_agent->id = atoi(buffer); }
					if(in_it_no) { current_foreignsector_agent->it_no = atoi(buffer); }
					if(in_day_of_month_to_act) { current_foreignsector_agent->day_of_month_to_act = atoi(buffer); }
					if(in_xgoods_unit_price) { current_foreignsector_agent->xgoods_unit_price = atof(buffer); }
					   ...
					   int partition_method, int flag)
{
    ...
    *itno = 0;
			...
			if(FLAME_in_outputs == 1)
			{
				if(FLAME_in_output == 1)
				{
					...
					if(FLAME_in_name == 1)
					{
						...
						else if(strcmp("foreignsector", buffer) == 0) current_FLAME_output->type = 7;
						else if(strcmp("jpoffice", buffer) == 0) current_FLAME_output->type = 8;
						else if(strcmp("mall", buffer) == 0) current_FLAME_output->type = 9;
						else if(strcmp("reagency", buffer) == 0) current_FLAME_output->type = 10;
						else 
						{
	...
	for(current_FLAME_output = FLAME_outputs; current_FLAME_output != NULL; current_FLAME_output = current_FLAME_output->next)
	{
		...
		else if(current_FLAME_output->type == 7) printf("agent' name='foreignsector");
		else if(current_FLAME_output->type == 8) printf("agent' name='jpoffice");
		else if(current_FLAME_output->type == 9) printf("agent' name='mall");
		else if(current_FLAME_output->type == 10) printf("agent' name='reagency");
...
void write_firm_agent(FILE *file, xmachine_memory_firm * current)
{
	...
	fputs("</bank_id>\n", file);
		fputs("<firmtype>", file);
	sprintf(data, "%i", current->firmtype);
	fputs(data, file);
	fputs("</firmtype>\n", file);
	...
	fputs("</capital_productivity_construction>\n", file);
		fputs("<physical_capital_construction>", file);
	sprintf(data, "%i", current->physical_capital_construction);
	fputs(data, file);
	fputs("</physical_capital_construction>\n", file);
		fputs("<physical_capital>", file);
	sprintf(data, "%i", current->physical_capital);
	fputs(data, file);
...
void write_foreignsector_agent(FILE *file, xmachine_memory_foreignsector * current)
{
	char data[1000];
	fputs("<xagent>\n" , file);
	fputs("<name>foreignsector</name>\n", file);
		fputs("<id>", file);
	sprintf(data, "%i", current->id);
	fputs(data, file);
	fputs("</id>\n", file);
	...
	fputs(data, file);
	fputs("</day_of_month_to_act>\n", file);
		fputs("<xgoods_unit_price>", file);
	sprintf(data, "%f", current->xgoods_unit_price);
	fputs(data, file);
	fputs("</xgoods_unit_price>\n", file);
...
void FLAME_write_xml(char * location, int iteration_number, int * output_types, int output_type_size)
{
	...
	if(FLAME_integer_in_array(0, output_types, output_type_size) || FLAME_integer_in_array(7, output_types, output_type_size))
	{
		current_xmachine_foreignsector_holder = foreignsector_start_state->agents;
			...
			{
				write_foreignsector_agent(file, current_xmachine_foreignsector_holder->agent);

				current_xmachine_foreignsector_holder = current_xmachine_foreignsector_holder->next;
			}
	}
