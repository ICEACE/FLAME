...
int read_loan_dynamic_array(char * buffer, int buffer_size, int * j, loan_array * temp_datatype_array)
{
	...
	while(buffer[(*j)] != '\0' && buffer[(*j)] != '}')
	{
		if(buffer[(*j)] == '{')
		{
			add_loan(temp_datatype_array, 0, 0.0);
...
int readEnvironmentXML(char * location)
{
	...
	int in_bank_risky_assets_ratio = 0;
	...
	while(c != (char)EOF)
	{
		...
		if(c == '>')
		{
			...
			if(strcmp(buffer, "bank_risky_assets_ratio") == 0) in_bank_risky_assets_ratio = 1;
			if(strcmp(buffer, "/bank_risky_assets_ratio") == 0) in_bank_risky_assets_ratio = 0;
		...
		else if(c == '<')
		{
			...
			if(in_environment == 1)
			{
				...
				if(in_bank_risky_assets_ratio == 1) { FLAME_environment_variable_bank_risky_assets_ratio = atof(buffer); }
				...
				int * itno)
{
	...
	while(c != (char)EOF)
	{
		...
		if(c == '>')
		{
			...
			if(strcmp(buffer, "revenue") == 0) { in_revenue = 1; }
			if(strcmp(buffer, "/revenue") == 0) { in_revenue = 0; }
			if(strcmp(buffer, "costs") == 0) { in_costs = 1; }
			if(strcmp(buffer, "/costs") == 0) { in_costs = 0; }
			...
			if(strcmp(buffer, "dividends") == 0) { in_dividends = 1; }
			if(strcmp(buffer, "/dividends") == 0) { in_dividends = 0; }
			if(strcmp(buffer, "fund_shares") == 0) { in_fund_shares = 1; }
			if(strcmp(buffer, "/fund_shares") == 0) { in_fund_shares = 0; }
			...
			if(strcmp(buffer, "n_dividends") == 0) { in_n_dividends = 1; }
			if(strcmp(buffer, "/n_dividends") == 0) { in_n_dividends = 0; }
		...
		else if(c == '<')
		{
			...
			if(FLAME_in_xagent == 1)
			{
				...
				else if(in_firm_agent == 1)
				{
					...
					if(in_revenue) { current_firm_agent->revenue = atof(buffer); }
					if(in_costs) { current_firm_agent->costs = atof(buffer); }
				 ...
				 }else if(in_household_agent == 1)
				{
					...
					if(in_dividends) { current_household_agent->dividends = atoi(buffer); }
					if(in_liquidity) { current_household_agent->liquidity = atof(buffer); }
					if(in_fund_shares) { current_household_agent->fund_shares = atof(buffer); }
				 ...
				 }else if(in_equityfund_agent == 1)
				{
					...
					if(in_n_dividends) { current_equityfund_agent->n_dividends = atoi(buffer); }
				 ...
				 }else if(in_bank_agent == 1)
				{
					...
					if(in_debt) { current_bank_agent->debt = atof(buffer); }
					   ...
					   int partition_method, int flag)
{
	...
	FLAME_environment_variable_bank_risky_assets_ratio = 0.0;
...
void write_firm_agent(FILE *file, xmachine_memory_firm * current)
{
	...
	fputs("</sales>\n", file);
		fputs("<revenue>", file);
	sprintf(data, "%f", current->revenue);
	fputs(data, file);
	fputs("</revenue>\n", file);
		fputs("<costs>", file);
	sprintf(data, "%f", current->costs);
	fputs(data, file);
	fputs("</costs>\n", file);
		fputs("<equity>", file);
...
void write_household_agent(FILE *file, xmachine_memory_household * current)
{
	...
	fputs(data, file);
	fputs("</housing_units>\n", file);
		fputs("<dividends>", file);
	sprintf(data, "%i", current->dividends);
	fputs(data, file);
	fputs("</dividends>\n", file);
		fputs("<liquidity>", file);
	sprintf(data, "%f", current->liquidity);
	fputs(data, file);
	fputs("</liquidity>\n", file);
		fputs("<fund_shares>", file);
	sprintf(data, "%f", current->fund_shares);
	fputs(data, file);
	fputs("</fund_shares>\n", file);
...
void write_equityfund_agent(FILE *file, xmachine_memory_equityfund * current)
{
	...
	fputs(data, file);
	fputs("</liquidity>\n", file);
		fputs("<n_dividends>", file);
	sprintf(data, "%i", current->n_dividends);
	fputs(data, file);
	fputs("</n_dividends>\n", file);
...
void FLAME_write_xml(char * location, int iteration_number, int * output_types, int output_type_size)
{
	...
	if(FLAME_integer_in_array(0, output_types, output_type_size))
	{
		...
		fputs("</household_budget_constraint>\n", file);
			fputs("<bank_risky_assets_ratio>", file);
		sprintf(data, "%f", FLAME_environment_variable_bank_risky_assets_ratio);
		fputs(data, file);
		fputs("</bank_risky_assets_ratio>\n", file);
